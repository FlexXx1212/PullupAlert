...
function renderOverview() {
  ...
  workouts.forEach((workout) => {
    ...
    if (isCompleted) {
      ...
      statusText = isPastView ? "Vergangener Tag" : "Geplant";
      statusEl.classList.add("workout-status--pending");
    }
    ...
    const canStart = isOnSelectedDay && isTodayView && !isCompleted;
...
function showActiveWorkout(workout) {
  ...
  const isOnSelectedDay = isWorkoutOnDate(workout, activeDate);
  const allowActiveControls = isOnSelectedDay && isViewingToday() && !isCompletedForDate;
